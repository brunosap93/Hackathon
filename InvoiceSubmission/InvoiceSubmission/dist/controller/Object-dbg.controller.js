	...
	return BaseController.extend("ns.InvoiceSubmission.controller.Object", {
        ...
        onAfterCheck: function(ruleResult){
            ...
            //debugger
                ...
                }else if(ruleResult == "Approval")
                {
                    var bCompact = !!this.getView().$().closest(".sapUiSizeCompact").length;
                    	MessageBox.warning(
                        "The net amount difference variance the selected invoice and the purchae order items is above the allowed threshold and it will require to be approved by "+ 
                                 ...
                                 if(sAction == "OK")
                                 {
                                    ...
                                    for(var i=0;i<selectedIndexes.length;i++)
                                    {
                                        ...
                                        if(oContext != undefined)
                                        {
                                            ...
                                            {
                                                POItems.push(String(oContext.getObject("PurchaseOrderItem")));
                                            }
                                        }
                                    }  
        ...
        oRowSelection : function(oEvent)
        {
            ...
            //debugger
                ...
                {
                    var oContext = oEvent.getSource().getContextByIndex(items[i]-1);
                    if(oContext != undefined)
                    {
                        var rowPath = String(oContext.sPath);
                        var idPO = parseInt(rowPath.split("/")[2]);

                        var oValue = this.getView().byId("treeTable").getModel().getData().value[idPO].To_PurchaseOrderItems[items[i]-1];
                        if(oValue != undefined)
                        {
